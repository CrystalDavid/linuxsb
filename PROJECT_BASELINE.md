...
> 阶段状态（2026-08-23）：P0 与 P0-B 均已在 API 26 真机通过。M1“只读产品纵向闭环”正在实现：正式应用壳、首页、主题详情、登录门控与回流已通过主机编译，30/30 单元测试通过并生成 signed HAP；`onDeviceTest` 与 API 26 真机 M1 手工闭环仍为 NOT RUN，因此尚未宣称 M1 完成。真实写操作继续为 NOT RUN。
...
真实写操作仍未授权和验证，继续保持功能开关关闭。P0-B 已通过；当前获准范围仅为 M1 只读产品纵向闭环，不扩展个人中心、搜索、正式写入能力或其他完整产品页面。
...
- 临时官方登录覆盖层与登录后同 topicId 刷新链路：PASS（单元契约与编译）；
- 原 19 个协议测试与新增 M1 测试：PASS（30/30）；
- signed HAP 构建：PASS；
- `hvigor onDeviceTest`：NOT RUN（当前无 HDC 真机目标）；
- API 26 真机默认首页、真实主题、匿名门控、登录回流和 5 条回复闭环：NOT RUN。

只有最后两项完成并确认 Web 节点 0、Cookie 泄露 0、`UI_FALLBACK` 0、Fatal 0、POST 0 后，M1 才能标记 PASS。
...
- 正式主题详情的技术阶段门已解除；当前 M1 任务只实现并验证只读首页 → 主题详情 → 登录回流纵向闭环。

### 通过

P0 只读主链路与 P0-B 同页回复契约均已通过并冻结；M1 只读纵向闭环正在真机验收，未完成前不得扩展后续功能。
