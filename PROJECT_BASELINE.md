...
> 阶段状态（2026-08-23）：P0、P0-B 与 M1“只读产品纵向闭环”均已在 API 26 真机通过。30/30 主机测试与 1/1 设备测试通过，signed HAP 构建、安装和启动成功；默认正式首页、真实主题、Topic 605 匿名门控、临时官方登录回流及登录后同页 5 条回复均已验证。普通页面 Web 节点、Cookie 泄露、`UI_FALLBACK`、Fatal 和 POST 均为 0。真实写操作继续为 NOT RUN。
...
- 正式主题详情的技术阶段门已解除；M1 已完成只读首页 → 主题详情 → 登录回流纵向闭环，后续范围尚未授权。

### 通过

P0 只读主链路、P0-B 同页回复契约与 M1 只读纵向闭环均已通过并冻结；后续功能必须另行定义范围和验收门槛。
