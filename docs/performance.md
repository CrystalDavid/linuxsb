...
性能优化不得通过删减业务内容、降低图片质量、改变字号/间距/颜色、替换冻结底栏或隐藏加载错误来“制造”更快的观感。正式可见界面保持 HarmonyOS 原生实现：ArkUI / UI Design Kit 负责界面，ArkWeb 同源 fetch 负责兼容 Cloudflare，TaskPool 负责 BBS1 解码；隐藏传输宿主不绘制业务 UI，只有登录/挑战需要操作时才显示网页。
...
- 首页恢复 ArkUI `List` 默认离屏缓存策略，不再固定预建 6 行；其它分页主题列表保留 3 个。主题行继续作为共享组件封装点击回调与两种固定排版，避免为复用装饰器引入函数参数和嵌套组件复用冲突。
- 首页滚动开始时取消预取，停止后从当前可见位置最多预取 3 个主题，延迟 900ms、最多 2 个并发 worker，避免预取和手势争抢。

### 2026-08-29 滚动热路径收敛

- 首页主题元信息由 `TopicPresentationMapper` 预计算，滚动建行不再重复进行相对时间解析与组合字符串计算；头像按显示尺寸请求解码。2026-08-30 参考 ArkDO 恢复首页默认离屏缓存，避免固定 6 行与首屏 12 个头像预取同时争抢桥接队列和图片解码；其它列表仍保持 3。
- 个人设置的 24 个预置头像从一次性远程 SVG 行改为水平 `List + LazyForEach`，仅缓存 2 个离屏项；DiceBear 请求改为 96px PNG，显示端固定按 48vp 解码。
- 我的称号、完整称号池和在售交易使用 `ArrayDataSource + LazyForEach`，页面切换与失败时主动释放旧数据源引用。
- HDS 标题栏渐进模糊半径由 24vp 收至 12vp；禁用态玻璃操作按钮不再维持背景采样。冻结底栏配方和几何未变。
- Pura 90 API 24 模拟器覆盖安装后，首页双向惯性滑动 3 轮、个人设置向下惯性滑动 4 轮、在售交易双向惯性滑动 3 轮，RenderService 三档 `>16.67 / 33 / 66ms` 均为 0。该结果只证明本轮模拟器回归未复现卡顿，API 26 真机 90/120Hz 仍需重测。
- “在售交易”双击设备回归产生一次 `/gacha_market?p=1` GET，返回 24 张卡并出现刷新提示；未产生 POST。

### 2026-08-30 Cloudflare 与首页滑动修复

- linux.sb 同源请求改由常驻隐藏 ArkWeb 宿主执行浏览器 `fetch`，避免 RCP 的 `HTTP 403 / Cf-Mitigated: challenge`；宿主始终位于不透明原生根壳之后，不新增可见层、点击命中或滚动容器。
- 隐藏宿主只加载同源的轻量 `/robots.txt` 引导文档，不再让完整论坛首页的 DOM、图片与动画常驻在原生列表背后；Cloudflare 挑战仍在同一路径返回的官方挑战文档中执行，可见人工挑战继续使用论坛首页。
- 同源头像改为浏览器会话获取后解码成 PixelMap，使用最多 5 个并发、在途合并与 160 项/48MiB 有界缓存；挑战恢复后可见头像会清除本轮失败标记并通过同一队列重试，避免首屏长期停在首字母占位，也避免列表复用期间重复触发被 Cloudflare 拒绝的原生图片请求和解码。
- 头像队列改为只由当前可见 `UserAvatar` 按需触发，不再一次性预取首页前 12 个 URL；桥接请求遇到同源 URL 重定向到不提供 CORS 的 CDN 时，按 ArkDO 的选路回退到 ArkUI `Image`，不再把仍可原生加载的头像永久判成失败。
- 共享玻璃仍保持 4vp / 1.42 / 0.98 / 原有低 alpha tint、尺寸和边框，但移除 `AdaptiveColor.AVERAGE` 的逐帧背景均色计算；HDS 标题栏移除 ArkDO 未使用的 `thermoCtrl`，保留设备能力探测后的 `ADAPTIVE` 系统材质与原渐进模糊。底栏组件源码和冻结视觉哈希均未修改。
- 同一台 Pura 90 API 24 模拟器空闲采样中，隐藏宿主加载完整论坛首页时 Web renderer RSS 约 747MiB、设备已用内存约 3.04GiB；改为 `/robots.txt` 后 renderer RSS 约 221MiB、设备已用内存约 2.14GiB，renderer RSS 下降约 70%。该数字只用于定位模拟器卡顿来源，不等同于真机 PSS。
- 修正后 Pura 90 API 24 冷启动截图在约 2.0 秒已出现首个真实头像，约 4.0 秒当前可见自定义头像基本到齐；Mate 80 Pro Max API 26 冷启动约 2.8 秒截图中当前可见自定义头像已显示。实时 feed 不同，因此这些只作为到达时延证据，不换算成严格百分比。
- Pura 90 API 24 在头像加载期间完成 4 轮完整双向惯性滑动，并连续快速切换根页面 12 次；Mate 80 Pro Max API 26 也连续快速切换 12 次。两台设备的应用表面 RenderService `>16.67 / 33 / 66ms` 三档均为 0；截图确认原首页几何、字体、底栏和主题行视觉未变。
- Mate 80 Pro Max API 26 的 HiSmartPerf 首页单次滑动实测 `90.6017fps / ResponseTime 36ms / HitchTimeRate 0.00ms/s / MAX_RENDER_SEQ_MISSED_FRAMES 0`，已低于用户提供的 35.0ms/s 卡顿率标准。Pura 90 API 24 因页面包含隐藏 ArkWeb，HiSmartPerf `slideList` 按工具限制返回 `-1`，因此模拟器只记录 RenderService 结果。AppGallery 自动化流畅性任务尚未用新包重跑，仍不得把本地 HiSmartPerf 结果写成 AppGallery 已通过；真实温控与弱网也未覆盖。

### 1.1.1 设置、投喂与 Cloudflare 性能路径

- 设置总览只增加一张两行静态卡片；投喂弹窗在用户点击时才创建，四个选项不发起网络、不查询商品，也不在滚动热路径绑定定时器或重型计算。
- 华为 IAP 环境检查和 `createPurchase` 只在用户点击具体金额后执行；页面加载、设置滚动、弹窗打开与关闭均不触发支付调用。
- 反馈入口复用现有原生主题详情和 TaskPool 解码链路，不创建 ArkWeb 或新的全量列表。
- 本轮 UI 功能在 API 24 模拟器完成可视确认；设置页长列表惯性滑动、深色模式、API 26 真机 90/120Hz 和支付收银台对性能的影响仍需按本文件同口径复验，当前为 `NOT RUN`。
...
6. 抓取新截图与 `dumpLayout`，确认普通页只有 1 个被遮挡且不可命中的传输 Web，可见业务 UI 未改；
