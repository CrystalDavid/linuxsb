...
| 沉浸光感材质 | PASS | 标题栏使用 HDS `GRADIENT_BLUR` 与能力探测后的系统材质；底栏、FAB、详情返回、分段控件、搜索/回复/发帖输入统一为一个 6vp `backgroundEffect` 采样层。全局默认使用 1.08 饱和度、1.12 亮度、约 1.25vp 高亮边缘和单圈中性阴影；深色资源中的底栏及通用玻璃高亮统一为 `#1AFFFFFF`（10% 白），替代此前过亮的 `#C2FFFFFF`。深色品牌蓝按用户指定保持 `#5B8FF9`；共享胶囊已取消白色填充，基底改为 `#18000000` 的低强度烟熏黑，选中层改为 `#0A5B8FF9` 的极淡品牌蓝，并用 `#245B8FF9` 低强度局部蓝光标记选中。方案参考华为应用市场底栏：烟熏底用于中和动态模糊采样到的白字和图片高光，选中态依靠蓝色图标/文字与轻蓝光，不再依赖发白的实心胶囊；实际动态玻璃以设备截图为准。顶部分段控件继续复用冻结底栏的 tint、边缘、阴影和移动胶囊参数并支持水平拖动；重复背景、第二模糊层和旧式暗染色均已删除 |
