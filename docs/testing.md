...
| 反馈路由 | 点击“反馈”，确认进入 `topic/17803` 的原生详情；正常页面只保留隐藏传输 Web，不出现可见网页业务 UI | API 24 模拟器历史 `PASS`；Cloudflare 改造后该入口 `NOT RUN` |
| 我的交易排版 | 检查发布卡字段顺序为“称号 / 单价 / 状态 / 编号日期”，并检查“交易记录”标题额外 8vp 顶部间距 | API 24 模拟器截图与布局树 `PASS` |
| Cloudflare 恢复与头像 | 冷启动、官方挑战、挑战完成自动返回原生首页、进程重启会话复用、当前可见头像按需加载 | Pura 90 API 24 `PASS`；Mate 80 Pro Max API 26 首页头像专项 `PASS` |
| 首页滑动与根页面切换 | 头像加载期间双向惯性滑动、连续快速切换根页；记录 RenderService 三档与真机 HiSmartPerf | Pura 90 与 Mate 80 Pro Max RenderService 三档均为 0；真机 `90.6017fps / 36ms / 0.00ms/s / 最大连续丢帧 0`；AppGallery 自动化复测 `NOT RUN` |
| 华为支付 | AGC 沙盒账号逐档打开系统收银台，覆盖取消、无效商品、成功购买和消耗确认 | `NOT RUN`：尚未配置/确认 AGC 商品与沙盒交易 |
| 邀请测试包 | release 签名 `.app` 的本地组装/校验、AGC 合法性解析、AppTest 安装启动 | 本地 `.app` / `.hap` 组装、`verify-app`、release Profile 与包内版本校验 `PASS`；AGC 上传、解析和 AppTest 安装 `NOT RUN` |

AGC 截图核对补充（2026-08-30）：软件包管理中已有 1.1.0（build 2）与 1.0.0（build 1）；1.1.0 合法性“已达标”、上架自检“待优化”。这不证明 1.1.1 已上传、解析或可安装。商品管理和四档 IAP 收银台端到端验收仍为 `NOT RUN`。
...
- `NoWebOutsideLogin` 将 Web 构造器锁定在官方登录/挑战页和隐藏同源传输宿主；
...
正常业务页面必须满足 Web 节点恰为 1，且该节点不可命中、被原生根壳完整遮挡；登录或 Cloudflare 挑战显示时允许临时为 2。挑战成功后应自动关闭可见页、重载当前原生数据并恢复 Web 1。
...
- Fatal：0
- 自动化真实 POST：0
- 正常业务页 Web 节点：1 个隐藏宿主；不得出现可见网页业务 UI
